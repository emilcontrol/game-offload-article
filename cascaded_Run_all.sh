...
mkdir -p results/cascadedResponse/sim1
        ...
        for nEDGE in "${nEDGESERVERS[@]}"; do
            for nANT in "${nANTENNAS[@]}"; do
                for nCLOUD in "${nCLOUDSERVERS[@]}"; do
                    ...
                    echo "python cascadedSimulate.py "$nDEV" "$nANT" "$nEDGE" "$nCLOUD" "results/cascadedResponse/sim1/${nDEV}agents_${nEDGE}edgeservers_${nANT}antennas_${nCLOUD}cloudservers_${i}.json""
                done
            done
        done
    done
